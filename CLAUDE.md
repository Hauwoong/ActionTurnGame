**⭑ 봉인의 두 종류 — 지속시간이 아니라 *부여 시점* 으로 갈린다 (2026-08-22 확정)**

| | 예 | 언제 적용 | 어떻게 |
|---|---|---|---|
| **딜레이 봉인** | 주사위·카드 효과 | **다음 턴** | 상태이상 + `delayed: true` |
| **즉발 봉인** | 유진 — **매 턴 최속을 잠그는 패시브** | **이번 턴** | 패시브가 턴 시작 훅에서 직접 잠근다 |

- **유진의 영구 봉인은 "영구 상태" 가 아니라 "매 턴 다시 걸리는 즉발 봉인" 이다 (사용자 확인).**
  매 턴 그때의 최속을 잠근다. 그래서 **"영구" 를 표현할 기계가 필요 없다 — 반복이 곧 영구다**
- ~~영구 쪽은 청사진 속성(`CharacterData` → `CharacterModel`)으로~~ — **접었다.**
  고정 슬롯이 아니라 매 턴 재선정이므로 청사진에 적을 것이 없다. 패시브 하나면 끝이고,
  `SpeedSlotPassive` 가 이미 같은 자리(`OnTurnStart`)에서 슬롯을 만지는 선례다

**⭑ 딜레이/즉발 두 갈래는 이미 만들어져 있다.**
`AddStatus(StatusEffectType type, int stack, bool delayed = false)` 의 그 `delayed` 가 정확히 이것이다
(3-2 Delay 작업의 산물, 2026-07-27). `DebugAddStrength` / `DebugAddDelayedStrength` 가 두 갈래를
찌르는 버튼까지 이미 있어서, `Seal` 을 `StatusEffectType` 에 하나 더하면 검증 진입점이 공짜로 생긴다.

**⭑ 턴 순서 — 이것이 설계의 뼈대다**

```
턴 시작:  봉인 전부 해제  →  RollSpeedDice()  →  TriggerTurnStart() 에서 다시 잠근다
                                                   (상태이상 루프 → 패시브 루프 순)
```

- **`IsSealed` 는 항상 한 턴짜리다.** 매 턴 해제되고 다시 걸린다. duration 기계가 필요 없고,
  `StatusEffectRuntime.Permanent`(= -1) 를 다른 뜻으로 재사용할 위험도 사라진다
- **봉인 슬롯도 속도를 굴린다.** 굴림이 봉인보다 **앞**이기 때문이다(잠글 때는 아직 아무것도 안 잠겨 있다).
  ~~"안 굴리는 쪽을 권한다"~~ 던 2026-08-22 초판 판단은 **뒤집힌다** — 대상 선정이
  "가장 빠른 슬롯" 인 이상 **전부 굴려놔야 고를 수 있다**
- 그래서 "봉인 슬롯의 속도를 굴리나" 는 취향 문제가 아니라 **규칙이 강제하는 것**이 됐다.
  화면에 숫자를 안 보여주는 것은 UI 선택이고 모델과 별개다
- 상태이상 루프가 패시브 루프보다 먼저 도므로 **딜레이 봉인이 즉발 봉인보다 먼저 적용된다.**
  둘 다 "가장 빠른 **비봉인** 슬롯" 을 고르면 겹치지 않고 두 개가 잠긴다

**남은 미확인 하나** — 해제 책임을 누가 지나. `RollSpeedDice` 앞에 해제 단계를 두는 것이 제일 단순한데,
`SpeedSlotRuntime.Roll()` 안에 넣으면 **봉인 슬롯을 안 굴리기로 바꾸는 순간 영영 안 풀린다**(순환).
옛 `Roll()` 이 `Used = false` 를 같이 하던 구조가 그 함정의 전형이었다.
...
- ~~**남은 것 — 지속시간**~~ — **2026-08-22 확정: 효과로 걸리는 봉인은 무조건 1턴.**
  (강력해서 제한을 둔 것으로 보인다). `Strength`/`Paralysis` 가 이미 duration 1 이라
  `StatusEffectRuntime` 의 기계가 그대로 받는다
- **단 영구 봉인도 있다 (유진).** 아래 "봉인의 두 종류" 참고 — **같은 기계로 묶으면 안 된다**
...
**⭑ 봉인의 두 종류 — 같은 기계로 묶으면 안 된다 (2026-08-22 사용자 추가)**

| | 예 | 지속 | 언제부터 |
|---|---|---|---|
| **효과로 걸리는 봉인** | 주사위/카드 | **1턴** | **다음 턴부터** |
| **영구 봉인** | 유진 — 한 슬롯을 아예 못 쓴다 | 영구 | **전투 시작부터** |

- **"다음 턴부터" 는 *거는 효과* 의 성질이지 *봉인 상태* 의 성질이 아니다.**
  유진 것에 그 지연을 그대로 적용하면 **1턴째가 안 잠긴다** — 규칙 위반이다.
  지연은 부여 경로에만 붙이고, `IsSealed` 자체는 "지금 잠겨 있나" 만 뜻해야 한다
- 그래서 영구 쪽은 **청사진 속성**(`CharacterData` → `CharacterModel` → `CharacterState`)으로
  슬롯을 만들 때 잠근 채로 만드는 것이 맞아 보인다. 상태이상으로 만들면 지연·duration·부여 시점을
  전부 예외 처리해야 한다
- **`StatusEffectRuntime.Permanent`(= -1) 를 재사용할 때 주의.** 지금 그 값의 뜻은
  "duration 으로 안 죽고 **스택이 떨어질 때까지**"(Bleed/Burn)다. 영구 봉인은 스택 개념이 없어
  **같은 상수인데 다른 뜻**이 된다 — `Ready` 가 두 뜻을 겸했던 것과 같은 병
- **미확인**: 유진의 영구 봉인이 **고정 슬롯**인가 **매 턴 최속**인가.
  봉인 슬롯은 어차피 맨 왼쪽에 깨진 채로 그려지므로 **화면으로는 구분이 안 된다.**
  게임적으로는 큰 차이다(매 턴 최속을 잃는 쪽이 훨씬 아프다).
  고정 슬롯이면 대상 선정 로직이 아예 필요 없어 청사진 속성 안이 더 맞아떨어진다
