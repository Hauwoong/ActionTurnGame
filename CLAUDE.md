...
### ▶ 다음 시작 지점 — **봉인 효과 + `_slotRuntimeMap` 버그** (한 덩어리. 아래 9번 항목)
...
2. 봉인 효과 + _slotRuntimeMap 버그   ← 여기부터. 설계 결정 3개와 에셋이 먼저 필요
3. 주석 38개 + 봉인이 추가한 메서드   (최종 형태에 대해 한 번)
```

**2번을 시작하기 전에 정해야 할 것 셋 (사용자 확인 필요)**
- **발동 조건** — 어떤 주사위/카드가 봉인을 거는가. 검증용 에셋을 만들어야 하니 첫 관문이다
- **지속시간** — 한 턴인가, 영구인가, 몇 턴인가. "가장 빠른 슬롯이 계속 잠긴다" 는 관측이
  *같은 슬롯이 계속* 인지 *매번 새로 최속을 잠그는지* 안 갈라준다
- **턴 중에 봉인되면 이미 등록된 액션은?** 해석 도중에 걸리면 그 슬롯에 카드가 올라가 있을 수 있다.
  취소되나, 그대로 수행되나
