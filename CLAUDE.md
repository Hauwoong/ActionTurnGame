...
**3-1.13 (힘/패시브가 `Counter` 를 안 올려주던 것)**, 그리고 검증 계기인 **전역 로그 콘솔 `DiceUI`**.
`_diceById` 도 첫 독자(`GetDiceInfo`)가 생겨 "쓰기 전용" 상태가 끝났다.

후보 (권장 순):

1. **`IsOffensive` 확장 메서드** (3-1.13 의 남은 절반, 사용자 결정으로 목록 포함).
   지금 힘·패시브 두 곳이 `!= Attack && != Counter` 인라인이고, `CombatExecutor.cs:162` /
   `DicePool.cs:81` 의 `!= Attack` 과 **모양이 같아서 뜻이 다르다는 게 안 보인다**.
   작고, 방금 겪은 버그의 재발을 막는다
2. **`BoutStartLog` / `BoutEndLog` 에 합 여부를 싣기** (아래 "미확인"). 지금
   `BoutEndEvent` 는 `DefenderId` 를 받아놓고 안 읽어서 **로그가 이벤트보다 정보가 적다**.
   3-1.11 과 같은 결이고, 이제 `DiceUI` 로 눈으로 확인할 수 있다
3. **`DamageLog` 에 공격자 추가** — `{TargetId, Amount}` 뿐이라 "누가 때렸나"가 없다.
   캐릭터별 UI 를 만들 때 반드시 걸린다(10번 참고)
4. **4번 2단계 — `CharacterModel` 신설.** 주사위·상태이상과 완전히 독립이라 머리를 쉬어가는 선택지.
   덩어리가 크다

1~3 은 전부 "로그가 사실을 다 담게 하기"라 한 덩어리로 묶어도 된다.
...
- **3-1.13. 힘이 카운터 주사위를 안 올려줬다 (2026-08-06 발견 → 같은 날 수정)**
  ...
  **← 남은 것: `IsOffensive` 확장 메서드.** 지금은 두 곳 다 `!= Attack && != Counter` 인라인이다.
  사용자 결정(2026-08-06): **만들 것 목록에 포함.** 근거는 아래 표 밑에 정리돼 있다.
...
- **`DamageLog` 에 공격자가 없다** (`{TargetId, Amount}`). 캐릭터별 패널을 만들면
  "내 주사위가 얼마 때렸나"가 **내 패널에 안 뜬다** — 굴림값과 데미지가 두 패널로 갈라진다.
  3-1.11 과 같은 종류의 결함이다(로그가 사실을 다 안 담는다). 전역 콘솔에서는 순서로 읽히므로
  급하지 않아 **별건으로 미뤘다** — 한 번에 둘을 바꾸면 회귀 원인을 구분할 수 없다는 기준
