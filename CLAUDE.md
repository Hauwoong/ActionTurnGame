...
### ▶ 내일 시작 지점 — 3-1.10 (`DiceRecoverEvent` 가 실제로는 효과가 없다)

**왜 이게 먼저인가.** 카드 부족은 해소됐고 **남은 주사위 수명 문제가 3-1.10 하나뿐이다.**
그리고 지금 관측 조건이 처음으로 갖춰졌다:

- `Guard`(Block 1개) / `Execute`(Attack·Block·Attack) 로 방어 주사위를 실제로 만들 수 있다
- `ResetDiceForNextTurn` 배선이 들어가서 커서 생애가 이제 턴 경계에서 실제로 끊긴다
  — 3-1.10 을 분석할 때 전제로 삼았던 것이 비로소 참이 됐다
- `DicePool.Recover()` 의 `foreach` 줄이 검증된 관측 지점이다(보관 분기 확인에 그대로 썼다)

**문제 요약.** 의도는 "일방에서 굴리지 않고 보관한 방어·회피 주사위를 나중에 꺼내 쓴다"인데 커서가 막는다:
- `Advance(Consume)` 는 `_cursor++` 를 하는데 `Recover()` 는 **상태만** 되돌리고 커서는 안 건드린다
- `Peek()` 은 `_cursor` 부터 **앞으로만** 훑고, `Inject` 는 새 주사위를 `_cursor` **위치에** 꽂는다
- 그래서 보관된 주사위는 `Ready` 로 돌아와도 그 턴 안에 다시 잡히지 않는다

**먼저 정할 것 (사용자 확인 필요)**: 원작에서 보관된 방어 주사위가 **언제** 다시 쓰이는가.
같은 턴 안의 다음 합인지, 아니면 애초에 "일방 공격을 당할 때"만인지. 이게 정해지지 않으면
커서를 어떻게 되돌릴지 결정할 수 없다 — `Recover()` 가 커서까지 되돌려야 하는지가 여기서 갈린다.
...
- **`ResetForNextTurn` 이 `_dice.Clear()` 를 해야 하나** — 누적은 확인됨(2턴차 `Count` 6).
  판단 근거는 DEVLOG `2026-07-30` 후속 섹션에 정리해뒀다. 안전성은 이미 확인
...
- 카드: `Guard`(Block 4~7) / `Combo`(Attack 1~3 ×3) / `Execute`(Attack 50~50, Block 4~7, Attack 3~6).
...
- **주사위 잔존과 리플레이 (2026-07-30 후속 세션, 설계 토론)** —
  ...
  - **미확인 전제**: "리플레이"가 (A) 처음부터 재생인지 (B) 중간 상태 저장/복원인지.
    (A)면 위 결론 그대로, (B)면 직렬화 대상 논의가 따로 필요하다(그때도 파괴된 주사위는 아니다).

### 3. 상태이상 — 남은 것

- **3-1.10. `DiceRecoverEvent` 가 실제로는 효과가 없어 보인다 (2026-07-30 발견, 미확인)** ← **다음 작업**
  ...
  - ~~`ResetForNextTurn` 이 `_cursor = 0` 과 함께 전부 `Destroy` 한다~~ — **초판의 이 전제는 틀렸다.**
    그때 `ResetForNextTurn` 은 호출자가 0건이라 아예 안 돌고 있었고, 보관된 주사위는 `Ready` 로
    영구히 남았다. 2026-07-30 후속 세션의 배선 복구로 **이제는 참이다**.
  - **먼저 정할 것 (원작 규칙, 사용자 확인 필요)**: 보관된 방어 주사위가 **언제** 다시 쓰이는가 —
    같은 턴의 다음 합인가, 아니면 "일방 공격을 당할 때"만인가. 이게 `Recover()` 가 커서까지
    되돌려야 하는지를 결정한다.
  - 관측은 `DicePool.Recover()` 의 `foreach` 줄에서. 3-1.6 보관 분기 검증에 쓴 자리 그대로다.
